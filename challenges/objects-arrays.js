...
for(let i = 0; i < graduates.length; i++){
  let names = {};
  names.first_name = graduates[i].first_name;
  names.email = graduates[i].email;
...
zooAnimals.forEach(type =>{
  animalNames.push(`Name: ${type.animal_name} Scientific: ${type.scientific_name}.`);
});
...
zooAnimals.filter(pop =>{
  lowerPopulation.push(pop.population < 5);
});
